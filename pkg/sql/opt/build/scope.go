...
func (s *scope) resolveColumnName(tblName optbase.TableName, colName optbase.ColumnName) *columnProps {
	...
	panic(fmt.Sprintf("unknown column %s", columnProps{name: colName, table: tblName}))
	return nil
}

func (s *scope) resolveColumnIndex(index xform.ColumnIndex) *columnProps {
	var aggScope *scope
	for curr := s; curr != nil; curr = curr.parent {
	...
	panic(fmt.Sprintf("unknown column index %s", index))
	return nil
}
...
func (s *scope) endAggFunc(agg xform.GroupID) (refScope *scope) {
	if !s.groupby.inAgg {
...
func (s *scope) IndexedVarResolvedType(idx int) types.T {
	...
	panic(fmt.Sprintf("unknown column index %s", idx))
	return nil
}
