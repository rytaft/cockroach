...
func (c *CustomFuncs) AppendFiltersItem(filters memo.FiltersExpr, item opt.ScalarExpr) memo.FiltersExpr {
	return append(filters, memo.FiltersItem{Condition: item})
}
...
// these functions.
func (c *CustomFuncs) FindConstrainedColumn(left, right opt.ScalarExpr) opt.ColSet {
	// Recurse over each conjunct from the left expression and determine whether
...
func (c *CustomFuncs) isSingleVariableConjunct(candidate, conjunction opt.ScalarExpr) (_ opt.ColSet, ok bool) {
...
// ScalarExprOuterCols calculates the set of outer columns from scratch for expressions
// that do not implement ScalarPropsExpr.
...
func (c *CustomFuncs) extractConjunctWithOuterCols(colSet opt.ColSet, expr opt.ScalarExpr) opt.ScalarExpr {
