...
func (h *Histogram) Intersect(other *Histogram) *Histogram {
	...
	return h.join(other, combineBuckets)
}
...
func (h *Histogram) InnerJoin(other *Histogram) *Histogram {
	combineBuckets := func(filteredLeft, filteredRight *cat.HistogramBucket) *cat.HistogramBucket {
		filteredBucket := cat.HistogramBucket{UpperBound: filteredLeft.UpperBound}
...
func (h *Histogram) join(
	...
	leftBucket := makeSpanFromBucket(&leftIter, tree.Datums{})
	rightBucket := makeSpanFromBucket(&rightIter, tree.Datums{})
	...
	bucIndex := sort.Search(leftCount, func(i int) bool {
		leftIter.setIdx(i)
		bucket := makeSpanFromBucket(&leftIter, tree.Datums{})
	...
	if bucIndex > 0 {
		...
		filtered.addEmptyBucket(prevUpperBound, false /* desc */)
	}
	...
	for ok := true; ok; ok = leftIter.next() {
		...
		// constraint library.
		left := makeSpanFromBucket(&leftIter, tree.Datums{})
		right := makeSpanFromBucket(&rightIter, tree.Datums{})
		...
		filteredSpan := left
		if !filteredSpan.TryIntersectWith(&keyCtx, &right) {
			filtered.addEmptyBucket(leftIter.b.UpperBound, false /* desc */)
			continue
		}
		...
		if filteredSpan.Compare(&keyCtx, &left) != 0 {
			...
			if cmpStarts != 0 {
				// We need to add an empty bucket before the new bucket.
				ub := leftIter.h.getPrevUpperBound(filteredSpan.StartKey(), filteredSpan.StartBoundary(), 0 /* colOffset */)
				filtered.addEmptyBucket(ub, false /* desc */)
			}
		}
		...
		filtered.addBucket(filteredBucket, false /* desc */)
